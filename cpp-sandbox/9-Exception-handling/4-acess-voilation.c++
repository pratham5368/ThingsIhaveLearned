/*An access violation is a specific type of error that occurs when a program attempts to access an illegal memory location. In C++, 
access violations are most commonly caused by:
...
1.Microsoft Visual Studio: Use the built-in debugger to identify the line of code responsible for the access violation error.

2.Valgrind: A popular Linux tool that detects memory leaks and access violations in your C++ programs.

3.AddressSanitizer: A runtime memory error detector for C++ that can detect out-of-bounds accesses, memory leaks, and use-after-free errors.*/